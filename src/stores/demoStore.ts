...
type DemoState = {
  ...
  saveDraftEdits: (input: { draftId: string; title: string; body: string }) => void
  ...
  persist(
    (set, get) => ({
      ...
      connectRedditAccount: (input) => {
        ...
        set((state) => ({ redditAccounts: [account, ...state.redditAccounts] }))
        return accountId
      },
      ...
      requestApproval: ({ taskId }) => {
        set((state) => ({
          ...
          drafts: state.drafts.map((draft) =>
            draft.taskId === taskId ? { ...draft, status: 'Needs approval' } : draft
          ),
        }))
      },
      ...
      resetDemo: () => {
        set({ projects: [], redditAccounts: [], roadmaps: [], tasks: [], drafts: [] })
      },
    }),
    {

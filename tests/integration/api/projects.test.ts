...
describe('Projects API (workspace-scoped)', () => {
  ...
  test('create -> list -> get -> patch -> delete', async () => {
    ...
    const createdJson = (await readJson(createdRes)) as { project: { id: string } }
    expect(createdJson?.project?.id).toBeTruthy()
    const projectId = createdJson.project.id

    const listRes = await listProjects(new Request('http://test.local/api/projects'))
    expect(listRes.status).toBe(200)
    const listJson = (await readJson(listRes)) as { items: Array<{ id: string }>; nextCursor: string | null }
    expect(listJson.items.some((p) => p.id === projectId)).toBe(true)

    const getRes = await getProject(new Request(`http://test.local/api/projects/${projectId}`), {
      params: { id: projectId },
    })
    expect(getRes.status).toBe(200)
    const getJson = (await readJson(getRes)) as { project: { id: string; name: string } }
    expect(getJson.project.id).toBe(projectId)
    expect(getJson.project.name).toBe('Test Project')

    const patchReq = new Request(`http://test.local/api/projects/${projectId}`, {
      method: 'PATCH',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ status: 'PAUSED' }),
    })
    const patchRes = await updateProject(patchReq, { params: { id: projectId } })
    expect(patchRes.status).toBe(200)
    const patchJson = (await readJson(patchRes)) as { project: { status: string } }
    expect(patchJson.project.status).toBe('PAUSED')

    const deleteRes = await archiveProject(new Request(`http://test.local/api/projects/${projectId}`, { method: 'DELETE' }), {
      params: { id: projectId },
    })
    expect(deleteRes.status).toBe(200)

    const archived = await prisma.project.findUnique({ where: { id: projectId }, select: { status: true } })
  ...
  test('list is scoped to workspace', async () => {
    ...
    const listRes = await listProjects(new Request('http://test.local/api/projects'))
    const listJson = (await readJson(listRes)) as { items: Array<{ id: string }> }

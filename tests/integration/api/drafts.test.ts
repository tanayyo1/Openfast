...
describe('Drafts APIs (variants + approval status)', () => {
  ...
  beforeAll(async () => {
    ...
    if (!ws) throw new Error('Seed workspace missing. Ensure prisma db seed ran.')
  ...
  test('draft lifecycle: create -> request -> approve', async () => {
    ...
    const createJson = (await readJson(createRes)) as { draft: { id: string; status: string } }
    expect(createJson.draft.status).toBe('DRAFT')
    const draftId = createJson.draft.id

    const reqRes = await requestApproval(new Request(`http://test.local/api/drafts/${draftId}/request-approval`, { method: 'POST' }), {
      params: { id: draftId },
    })
    ...
    const approveRes = await approveDraft(new Request(`http://test.local/api/drafts/${draftId}/approve`, { method: 'POST' }), {
      params: { id: draftId },
    })
    expect(approveRes.status).toBe(200)
    const approveJson = (await readJson(approveRes)) as { draft: { status: string; approvedBy: string } }
    ...
    const getRes = await getDraft(new Request(`http://test.local/api/drafts/${draftId}`), { params: { id: draftId } })
  ...
  test('draft lifecycle: request -> reject', async () => {
    ...
    await requestApproval(new Request(`http://test.local/api/drafts/${draftId}/request-approval`, { method: 'POST' }), {
      params: { id: draftId },
    })
    ...
    const rejectJson = (await readJson(rejectRes)) as { draft: { status: string } }

...
describe('Roadmaps + tasks APIs (workspace-scoped)', () => {
  ...
  test('create -> list -> get -> list tasks -> update task status', async () => {
    ...
    const createJson = (await readJson(createRes)) as { roadmap: { id: string }; tasksCreated: number }
    expect(createJson.tasksCreated).toBe(3)
    roadmapId = createJson.roadmap.id

    const listRes = await listRoadmaps(new Request('http://test.local/api/roadmaps'))
    ...
    const getRes = await getRoadmap(new Request(`http://test.local/api/roadmaps/${roadmapId}`), {
      params: { id: roadmapId },
    })
    expect(getRes.status).toBe(200)

    const tasksRes = await listTasks(new Request(`http://test.local/api/roadmaps/${roadmapId}/tasks`), {
      params: { id: roadmapId },
    })
    expect(tasksRes.status).toBe(200)
    const tasksJson = (await readJson(tasksRes)) as { items: Array<{ id: string }> }
    ...
    const taskRes = await getTask(new Request(`http://test.local/api/tasks/${taskId}`), {
      params: { id: taskId },
    })
    expect(taskRes.status).toBe(200)
    const taskJson = (await readJson(taskRes)) as { task: { status: string; completedAt: string | null } }

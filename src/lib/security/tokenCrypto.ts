...
export class TokenCryptoError extends Error {
  code: "MISSING_KEY" | "INVALID_KEY" | "INVALID_PAYLOAD" | "DECRYPT_FAILED";
...
export function encryptToken(
  plaintext: string,
  opts?: { keyId?: string; keyring?: Keyring },
) {
  ...
  if (!keyId) {
    throw new TokenCryptoError(
      "MISSING_KEY",
      "No token encryption keys configured",
    );
  }
  ...
  if (!key || key.length !== 32) {
    throw new TokenCryptoError(
      "INVALID_KEY",
      `Invalid token encryption key: ${keyId}`,
    );
  }
...
export function decryptToken(payload: string, opts?: { keyring?: Keyring }) {
  ...
  if (!key || key.length !== 32) {
    throw new TokenCryptoError(
      "MISSING_KEY",
      `Missing token encryption key: ${keyId}`,
    );
  }

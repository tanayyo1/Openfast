...
export async function POST(req: Request) {
  ...
  const project = await prisma.project.findFirst({
    where: { id: projectId, workspaceId: session.workspaceId, status: { not: 'ARCHIVED' } },
    select: { id: true },
  })
  if (!project) {
  ...
  const created = await prisma.$transaction(async (tx: Prisma.TransactionClient) => {
    const roadmap = await tx.roadmap.create({
      ...
      select: { id: true, projectId: true, startDate: true, horizonDays: true, status: true, createdAt: true, updatedAt: true },
    })
  ...
  return NextResponse.json({ roadmap: created.roadmap, tasksCreated: created.tasksCreated }, { status: 201 })
}

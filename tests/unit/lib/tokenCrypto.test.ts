import { decryptToken, encryptToken, redactSecret, TokenCryptoError } from '@/lib/security/tokenCrypto'
...
describe('tokenCrypto', () => {
  ...
  test('decrypt fails with wrong key', () => {
    ...
    expect(() => decryptToken(payload, { keyring: { v1: keys.v2 } })).toThrow(TokenCryptoError)
  })

  test('invalid payload format throws', () => {
    const keys = keyring({ v1: Buffer.alloc(32, 1) })
    expect(() => decryptToken('bad', { keyring: keys })).toThrow(TokenCryptoError)
  })
